...
notebook/Data Analysis.ipynb
        │
        └──► data/new_data/roads_after_dataanalysis.csv (vulnerability + criticality scores per LRP, intermediate, after merge step)
...
model/model.py  (reads roads_after_dataanalysis.csv, selects roads based on configuration)
        │
        └──► data/new_data/roads_after_preprocessing.csv   (filtered network used in simulation)
